...
Reference imagery for visual digitisation was originally specified as PlanetScope NICFI (3 m, 4-band, 6-monthly mosaics over Asia). Following the discontinuation of the NICFI Education and Research Basic Account programme and its replacement by the Tropical Forest Observatory (Planet Labs Inc., correspondence 5 May 2026, ticket #196369), eligibility was restricted to forest, climate, and biodiversity monitoring use cases and the present agricultural use case did not qualify. A second-line appeal was lodged with KSAT (Kongsberg Satellite Services), the ground-station operator behind the Tropical Forest Observatory programme. Conditional on KSAT's ruling, the v3 classifier uses one of two reference-imagery configurations:

- **Configuration A (NICFI granted):** 3 m PlanetScope basemaps as the primary visual reference, with Sentinel-2 used as a context layer.
- **Configuration B (NICFI denied):** 10 m Sentinel-2 SR Harmonized cloud-free median composites over the cyclone landfall ±15-day window as the primary reference. Sentinel-2's SWIR bands (B11, B12) provide a stronger salinity signature than NICFI lacks; the §E5 fallback is therefore not a degradation in classifier capacity but rather a trade-off of spatial resolution against spectral information.

The OSF working project (osf.io/3vua4) records the activation date of Module 02b (5 May 2026, 13:50 IST), the NICFI / TFO correspondence, the KSAT appeal, and the final imagery-configuration decision.
