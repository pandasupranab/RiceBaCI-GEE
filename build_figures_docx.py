...
FIGURES = [
    ...
    (
        ...
        "Two-way fixed-effects difference-in-differences estimates for τ_SOS, τ_POS, and τ_EOS on the raw phenology panel (orange) and the classifier-corrected panel (blue). Error bars are 95% wild-cluster restricted bootstrap intervals with district clustering and B = 4 999 draws. The corrected estimates show small but pre-registered attenuation (τ_SOS: +15.289 → +15.108 days; τ_EOS: 0.000 → −0.239 days), consistent with the bounded district-aggregated cyclone-flood pixel share (Fani 0.04–2.5 %, Amphan 0.005–1.9 %, Yaas 0.018–7.2 %).",
        ...
        "TWFE = two-way fixed effects; DiD = difference-in-differences; τ = DiD treatment-effect estimate (days); τ_SOS / τ_POS / τ_EOS = τ on the start, peak, and end of the rice-cropping season; SOS / POS / EOS = start, peak, and end of season; WCB = wild-cluster (restricted) bootstrap; B = number of bootstrap draws; CI = confidence interval (95 % WCB-restricted, district-clustered); SE = standard error of τ-hat (district-clustered; the half-width of the displayed error bars is derived from SE · t-critical); days = calendar days (effect-size unit); % = per-cent (district-aggregated pixel-share unit); v2.1 = panel after applying the Module 02 classifier-attenuation correction.",
    ),
    (
        "Figure 3",
    ...
    (
        ...
        "Statistical power as a function of the true τ_SOS effect size for the v2.1 corrected panel, computed under district clustering with N_clusters = 8 and N_periods = 8 (solid line; G = 8 this study). Comparison curves at G = 4, 6, and 12 districts illustrate the impact of cluster count on detectable effect. The plot annotation 'Observed |tau-hat| range (strong cells): 1.9-5.7 d' refers to the |τ-hat| range across the subset of strong-effect district-event cells; the all-cells DiD estimate is |τ-hat_SOS| ≈ 15 days. Post-hoc power at this realised effect is 0.18, and MDE_80% is approximately 35 days under one-sided α = 0.05.",
    ...
    (
        ...
        "τ-hat = estimated DiD treatment effect (days); tau_real = τ-hat from the real coastal/inland assignment; SOS / POS / EOS = start, peak, and end of season; DiD = difference-in-differences; p = probability value (significance level); p_perm = permutation (donor-swap) p-value; n = number of donor-swap permutations (here n = 55, after excluding the true assignment from C(8, 5) = 56); n_extreme = count of donor-swap permutations with |τ-hat_placebo| ≥ |τ-hat_real|; C(8, 5) = binomial coefficient (number of ways to choose 5 treated districts from 8); days = calendar days (effect-size unit); v1 = pre-classifier phenology pipeline; v2.1 = classifier-corrected pipeline.",
    ),
    (
        "Figure S1",
    ...
    (
        ...
        "Mean σ⁰_VV and σ⁰_VH (dB) time-series for three flood-class archetypes derived from the n = 480 reference-label panel: (A) agronomic transplanting flooding (freshwater, controlled depth ~5–15 cm, ΔVH ≈ −6.5 dB), (B) cyclone saline storm-surge inundation (impulsive onset, deep flood 0.5–3 m, ΔVH ≈ −10.5 dB), and (C) freshwater rainfall ponding (Bulbul-class, post-monsoon shallow water, ΔVH ≈ −3.0 dB), over the 60-day window centred on the cyclone landfall date. Saline-surge pixels show a faster, deeper trough and a slower recovery than agronomic flooding — the physical basis for the classifier's separability and the OA = 0.844 SAR-only robustness variant.",
        ...
        "SAR = synthetic aperture radar; σ⁰ = normalised radar backscatter coefficient (linear power form, displayed in dB); dB = decibels (logarithmic backscatter unit, 10 · log10[σ⁰]); VV = vertical-transmit, vertical-receive co-polarisation channel; VH = vertical-transmit, horizontal-receive cross-polarisation channel; CR = cross-polarisation ratio (VH − VV in dB); ΔVH = post-onset minus pre-onset σ⁰_VH; n = 480 reference-label panel (80 Copernicus EMS EMSR357 Fani cyclone-flood + 160 UN-SPIDER Sentinel-1 change-detection Amphan/Yaas cyclone-flood + 240 Sentinel-1∩WorldCover∩JRC agronomic-flood); days = calendar days (time-series x-axis unit, ±30 d window around landfall); OA = overall accuracy (fraction of correctly classified test pixels); v0.3.0 = released version of the Module 02 random-forest classifier.",
    ),
]
